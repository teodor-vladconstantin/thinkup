...
import styles from "../../../styles/LinkAccountPopUp.module.css";
...
const LinkAccountPopUp = ({ className, close, account_type }) => {
    ...
    useEffect(()=>{
        ...
        if(User.social_connections[account_type]!=undefined)
            setLink(User.social_connections[account_type]);
    ...
    return (
        <PopUpContainer className={styles.LinkAccountPopUp + " " + className}>
            <p>
                Link your <span>{account_type}</span> account
            </p>
            <InputField
                width="675"
                value={Link}
            ...
            />
            {User.social_connections[account_type]!=undefined?<p className={styles.DeleteSocialLink} onClick={()=>removeLink()} >Delete Connection</p>:<></>}

            <div className={styles.AddLinkButtons}>
                <AccentButton text="Add " onClick={() => AddLink()}>
                    <svg
                        width="21"
                        height="22"
                        viewBox="0 0 21 22"
