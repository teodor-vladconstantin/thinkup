...
### Găsite în timpul testării de mai sus, NEREZOLVATE (nu erau în scope, de discutat separat)
- **Parola Gmail din `.env` e respinsă de Google**: `SMTPAuthenticationError: (535, 'Username and Password not accepted' ...)` la un test real de trimitere prin `/contact` cu variabila prezentă. Nu e legat de niciunul din fix-urile de mai sus — App Password-ul Gmail curent pare expirat/revocat. Formularul de contact e efectiv nefuncțional în producție până când cineva generează un App Password nou în contul Google și îl pune în `.env`. (Fix-ul de mai sus tot ajută: acum eroarea asta apare clar în log, cu traceback, nu ca "ok" fals.)
- **`API_CRUD_REVIEWS.addReview` / `deleteReview`** (`api_crud_reviews.py`) — `projJson["projectReviews"]["total_reviews"] += 1` (și `-= 1` la delete) crapă cu `TypeError` pentru proiecte unde `total_reviews`/`average_rating` sunt stocate ca string (`"0"`) în loc de număr — cazul pentru cel puțin un proiect real din DB. `POST /projects/<id>/addReview/<reviewId>` și `DELETE /reviews/delete/...` întorc 500 pe acele proiecte. Descoperit încercând să testez fix-ul de mai sus prin fluxul normal de creare a unui review; am ocolit bug-ul semănând review-ul direct în DB pentru a testa doar `updateReview`.
