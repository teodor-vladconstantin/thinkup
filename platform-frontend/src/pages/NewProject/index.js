...
import FileUploadInput from "../../components/FormElems/FileUploadInput";
...
const NewProject = () => {
            ...
            uploadPhotoToCloudinary(entry.file, CreatedProjectId)
                .then((url) => {
                    setPhotoEntries((prev) =>
                        prev.map((p) =>
                            p.id === entry.id ? { ...p, status: "done", url } : p
                        )
                    );
                })
                .catch((err) => {
    ...
    const savePhotosAndFinish = async () => {
        const anyUploading = PhotoEntries.some((p) => p.status === "uploading");
        if (SavingPhotos || anyUploading) return;

        const uploadedUrls = PhotoEntries.filter((p) => p.status === "done").map(
            (p) => p.url
        );
        ...
        try {
            ...
            const response = await apiClient.put(
                `${process.env.NEXT_PUBLIC_API_URL}/projects/${CreatedProjectId}`,
                formdata
            );
    ...
    if (CreatedProjectId) {
        const errorCount = PhotoEntries.filter((p) => p.status === "error").length;
        const anyUploading = PhotoEntries.some((p) => p.status === "uploading");
        ...
        return (
            <ScrollContainer className={styles.Contact}>
                ...
                <h1>Adaugă poze proiectului</h1>
                <h4>
                    Proiectul a fost creat. Poți adăuga poze acum sau mai târziu,
                    din pagina proiectului.
                </h4>

                <FileUploadInput
                    FileUploadTitle="Poze proiect"
                    fileChange={handlePhotosSelected}
                />
                            ...
                            <div key={entry.id} className={styles.photoThumb}>
                                ...
                                <span
                                    className={
                                        ...
                                        " " +
                                        (entry.status === "done"
                                            ? styles.photoStatusDone
                                            : entry.status === "error"
                                            ? styles.photoStatusError
                                            : styles.photoStatusUploading)
                                    }
                                >
                                    {entry.status === "uploading" && "Se încarcă..."}
                                    {entry.status === "done" && "✓"}
                                    {entry.status === "error" && "Eroare"}
                                </span>
                            </div>
                        ))}
                    </div>
                ...
                {errorCount > 0 && (
                    <div className={styles.error} style={{ display: "block" }}>
                        {errorCount} poză(e) nu s-au putut încărca. Poți continua cu
                        restul sau le poți selecta din nou.
                    </div>
                )}
                ...
                {PhotosSaved && (
                    <p className={styles.success}>Poze salvate cu succes!</p>
                )}
                ...
                <div className={styles.btnDiv}>
                    ...
                    <CancelButton text="Sări peste" onClick={() => router.push("/")} />
                </div>
            </ScrollContainer>
        );
    }
